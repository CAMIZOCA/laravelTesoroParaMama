        ...
        <form method="POST" action="{{ route('admin.ambassadors.update', $ambassador) }}" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $ambassador->name) }}"
                ...
                <div>
                    ...
                    <input type="text" id="last_name" name="last_name" value="{{ old('last_name', $ambassador->last_name) }}"
                           class="form-input">
            ...
            <div>
                ...
                <input type="email" id="email" name="email" value="{{ old('email', $ambassador->email) }}"
            ...
            <div>
                ...
                <input type="text" id="code" name="code" value="{{ old('code', $ambassador->code) }}"
                       class="form-input uppercase font-mono" style="text-transform:uppercase" required>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="discount_type" name="discount_type" class="form-input" required>
                        <option value="percentage" {{ old('discount_type', $ambassador->discount_type) === 'percentage' ? 'selected' : '' }}>Porcentaje (%)</option>
                ...
                <div>
                    ...
                    <input type="number" id="discount_value" name="discount_value"
                           value="{{ old('discount_value', $ambassador->discount_value) }}" min="0" step="0.01"
                           class="form-input" required>
            ...
            <div>
                ...
                <select id="status" name="status" class="form-input" required>
                    <option value="active"   {{ old('status', $ambassador->status) === 'active'   ? 'selected' : '' }}>Activo</option>
                    <option value="inactive" {{ old('status', $ambassador->status) === 'inactive' ? 'selected' : '' }}>Inactivo</option>
                </select>
                @error('status') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="flex gap-3 pt-2">
                <button type="submit" class="admin-btn-primary px-6 py-2">Guardar cambios</button>
